...
func Test_splitYAMLDocs_WhenSingleDocument_ItShouldReturnOneDoc(t *testing.T) {
	input := []byte("kind: ConfigMap\nmetadata:\n  name: foo")
	docs := splitYAMLDocs(input)
	assert.Len(t, docs, 1)
}

func Test_splitYAMLDocs_WhenMultipleDocuments_ItShouldReturnAllDocs(t *testing.T) {
	input := []byte("kind: ClusterRole\n---\nkind: ClusterRoleBinding")
	docs := splitYAMLDocs(input)
	assert.Len(t, docs, 2)
}

func Test_splitYAMLDocs_WhenEmptySeparator_ItShouldSkipEmptyDocs(t *testing.T) {
	input := []byte("kind: ClusterRole\n---\n\n---\nkind: ClusterRoleBinding")
	docs := splitYAMLDocs(input)
	assert.Len(t, docs, 2)
}
...
func Test_buildSecret_WhenCalled_ItShouldSetCorrectFields(t *testing.T) {
	secret := buildSecret("my-secret", "my-ns", map[string][]byte{"key": []byte("value")})
	assert.Equal(t, "my-secret", secret.Name)
	assert.Equal(t, "my-ns", secret.Namespace)
...
func Test_handleRoute_WhenSpokeNotAvailable_ItShouldReturn503(t *testing.T) {
	...
	p := newTestProxy(t, mc)
	w := httptest.NewRecorder()
	path := "/apis/" + hcpProxyAPIGroup + "/" + hcpProxyAPIVersion + "/namespaces/clusters/hostedclusters?hostingCluster=spoke-1"
...
func Test_checkHubPermission_WhenViewOnlyUser_ItShouldReturnError(t *testing.T) {
	...
	hubSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Header.Get("Impersonate-User") != "" {
			w.WriteHeader(http.StatusNotFound)
			_, _ = io.WriteString(w, `{"kind":"Status","apiVersion":"v1","reason":"NotFound","message":"userpermissions.clusterview.open-cluster-management.io \"managedcluster:admin\" not found"}`)
			return
		}
		if strings.Contains(r.URL.Path, "userpermissions/managedcluster:admin") {
			w.Header().Set("Content-Type", "application/json")
...
func Test_checkHubPermission_WhenClusterviewAPIAbsent_ItShouldSkipAndAllow(t *testing.T) {
	...
	hubSrv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusNotFound)
		_, _ = io.WriteString(w, `{"kind":"Status","apiVersion":"v1","reason":"NotFound","message":"the server could not find the requested resource"}`)
	}))
	defer hubSrv.Close()
...
func Test_handlePatchResources_WhenFullBundleSent_ItShouldPutHCAndNPsAndReturnBundle(t *testing.T) {
	...
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPatch, "/", bytes.NewReader(reqBody))
	r.Header.Set("X-Remote-User", "alice")
...
func Test_handleRoute_WhenResourcesSubresourcePatch_ItShouldDispatch(t *testing.T) {
	...
	path := "/apis/" + hcpProxyAPIGroup + "/" + hcpProxyAPIVersion +
		"/namespaces/clusters/hostedclusters/my-hc/resources?hostingCluster=spoke-1"
	w := httptest.NewRecorder()
	r := httptest.NewRequest(http.MethodPatch, path, bytes.NewReader(reqBody))
	r.Header.Set("X-Remote-User", "alice")
	r.Header.Set("Content-Type", "application/json")
	p.handleRoute(w, r)
...
func Test_handleRoute_WhenPatchOnNamedResource_ItShouldDoBundleReplace(t *testing.T) {
	// PATCH /{name} (no /resources suffix) must behave identically to PATCH /{name}/resources —
...
func Test_applyService_WhenServiceDoesNotExist_ItShouldCreate(t *testing.T) {
	p := newTestProxy(t)
	svc := &corev1.Service{
		ObjectMeta: metav1.ObjectMeta{Name: "test-svc", Namespace: "multicluster-engine"},
		Spec: corev1.ServiceSpec{
			Selector: map[string]string{"app": "test"},
			Ports:    []corev1.ServicePort{{Port: 443}},
		},
	}
	err := applyService(context.Background(), p.hubClient, svc)
	assert.NoError(t, err)

	got := &corev1.Service{}
	err = p.hubClient.Get(context.Background(),
		types.NamespacedName{Name: "test-svc", Namespace: "multicluster-engine"},
		got)
	assert.NoError(t, err)
	assert.Equal(t, "test", got.Spec.Selector["app"])
}

func Test_applyService_WhenServiceAlreadyExists_ItShouldUpdate(t *testing.T) {
	...
	p := newTestProxy(t, existing)
	updated := existing.DeepCopy()
	updated.Spec.Selector = map[string]string{"app": "new"}

	err := applyService(context.Background(), p.hubClient, updated)
	assert.NoError(t, err)

	got := &corev1.Service{}
	_ = p.hubClient.Get(context.Background(),
		types.NamespacedName{Name: "test-svc", Namespace: "multicluster-engine"},
		got)
	assert.Equal(t, "new", got.Spec.Selector["app"])
}

// --- applyRBAC ---

func Test_applyRBAC_WhenValidYAML_ItShouldCreateClusterRoleAndBinding(t *testing.T) {
	p := newTestProxy(t)
	rbacYAML := []byte(`apiVersion: rbac.authorization.k8s.io/v1
kind: ClusterRole
metadata:
  name: test-role
rules:
...
- kind: ServiceAccount
  ...
  namespace: multicluster-engine`)
	...
	err := p.applyRBAC(context.Background(), rbacYAML, "my-ns")
	assert.NoError(t, err)

	cr := &rbacv1.ClusterRole{}
	err = p.hubClient.Get(context.Background(),
		types.NamespacedName{Name: "test-role"}, cr)
	assert.NoError(t, err)

	crb := &rbacv1.ClusterRoleBinding{}
	err = p.hubClient.Get(context.Background(),
		types.NamespacedName{Name: "test-binding"}, crb)
	assert.NoError(t, err)
	// namespace should be overridden to the operator namespace
...
// The mock server URL is used as the spoke base (spokeName segment is appended).
func newTestProxyWithSpokeURL(t *testing.T, spokeServerURL string, objs ...runtime.Object) *hcpProxy {
	...
	// Strip the spoke-name segment from the URL by setting the base to the
	// server root — the test server ignores path prefixes.
