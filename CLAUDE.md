...
Both `pkg/manager/manifests/hcp-proxy/rbac.yaml` and the backplane-operator ClusterRole must include:
```yaml
...
The hub manager pod (Deployment, ClusterRole, ServiceAccount) is **not** defined in
this repo. It is defined in:
